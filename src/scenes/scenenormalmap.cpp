...
{
    ogre = ObjMesh::load("src/media/bs_ears.obj",false,true);
}
...
void SceneNormalMap::initScene()
{
    ...
    view = glm::lookAt(vec3(-1.0f,-0.35f,2.0f), vec3(0.0f,0.0f,0.0f), vec3(0.0f,1.0f,0.0f));
    ...
    GLuint diffTex = Texture::loadTexture("src/media/texture/ogre_diffuse.png");

    // Load normal map
    GLuint normalTex = Texture::loadTexture("src/media/texture/ogre_normalmap.png");
...
#else
    prog.compileShader("src/textures/shader/normalmap.my.vert.glsl");
    prog.compileShader("src/textures/shader/normalmap.frag.glsl");
#endif
    prog.link();
    prog.use();
