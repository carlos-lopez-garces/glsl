...
SceneReflectCube::SceneReflectCube() : angle(0.0f), tPrev(0.0f), rotSpeed(glm::pi<float>() / 8.0f),
                                       teapot(14, glm::mat4(1.0f)), sky(30.0f)
{ }
...
void SceneReflectCube::setMatrices(GLSLProgram &p)
{
    ...
    p.setUniform("NormalMatrix", glm::inverseTranspose(glm::mat3(mv)));
}
...
{
	try {
		prog.compileShader("shader/cubemap_reflect.my.vs");
		prog.compileShader("shader/cubemap_reflect.my.fs");
