...
void SceneDiffIbl::render()
{
	...
	prog.setUniform("Material.Kd", 0.9f);
...
{
	try {
		prog.compileShader("shader/diffuseibl.my.vert.glsl");
		prog.compileShader("shader/diffuseibl.my.frag.glsl");
		prog.link();

		sbProg.compileShader("shader/skybox.my.vs");
		sbProg.compileShader("shader/skybox.my.fs");
		sbProg.link();
	}
